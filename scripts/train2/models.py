...
class DopeResNet18(nn.Module):
    def __init__(
        ...
        super(DopeResNet18, self).__init__()
       

        # # modify full connect output layer
        ...
        # self.upsample.add_module('16', nn.Conv2d(160, 64,
        #     kernel_size=3, stride=1, padding=0))
        self.upsample.add_module('55',InvertedResidual(512, 320, stride=1, expand_ratio=6, norm_layer=nn.BatchNorm2d))
        self.upsample.add_module('56',InvertedResidual(320, 64, stride=1, expand_ratio=6, norm_layer=nn.BatchNorm2d))
    ...
    def forward(self, x):
        '''Runs inference on the neural network'''
        # print(x.shape)
        out_features = self.resnet_feature(x)
        # print('out2_features',out_features.shape)
        output_up = self.upsample(out_features)
        # print('output_up',output_up.shape)
...
class DopeResNet50(nn.Module):
    def __init__(
        ...
        super(DopeResNet50, self).__init__()
...
#EfficientNet b0
class DopeEfficientNet_B0(nn.Module):
    def __init__(
            self,
            pretrained=False,
            numBeliefMap=9,
